...
## file use: Produce exploratory UA commencement, major, honors, recruiting,
## and post-graduation descriptives used while developing the out-of-state
## enrollment project. Production town-level recruiting regressions now live in
## code/crossan/recruiting/town_matching; this script preserves the legacy
## exploratory workflow.
#
## inputs:
## 1. data/all_alabama_data.csv -- curated UA commencement analysis file
## 2. data/recruiting_data_ozan.csv -- external cleaned recruiting comparison file
## 3. data/full_ua_recruiting_scrape.csv -- self-produced admissions visit scrape
## 4. data/bama_highschool_2011_2024.csv -- UA high-school feeder panel
## 5. data/linked_commencement_revelio_profile_data.csv -- commencement-Revelio links
## 6. revelio_data/first_spell_join.rds -- first observed post-graduation job spell
#
## outputs:
## 1. figures/descriptive-figs/*.png -- descriptive commencement and recruiting plots
## 2. figures/recruiting_iv.png -- exploratory 2017 visit-count cross-section
## 3. figures/recruiting_iv_changes.png -- exploratory visit-count change plot
#=====================================================================
...
# Use the shared path config so code can live under code/crossan while reading
# data from the original admissions_project directory. Set
# ADMISSIONS_PROJECT_DATA_ROOT to override the default data root on another
# machine.
...
### i. read curated commencement records

## (a) This locked input is generated by commencement_revelio_link_v1.R and is
##     still the analysis source for the legacy descriptive workflow.
ua_dt <- fread(file = file.path(pathData, "all_alabama_data.csv"))

### ii. edit degree and honors information

## (a) Collapse noisy degree text into broad fields used in descriptive figures.
...
## (b) Collapse the heterogeneous Honors field into mutually exclusive buckets.
...
## (c) Flag any honors designation for pooled honors-rate descriptives.
...
## (a) Use the raw state field here to preserve the original descriptive coding.
...
## (a) Trim punctuation and whitespace before constructing town-level panels.
ua_dt[, OriginState := trimws(gsub("[.,]", "", `Origin State`))]

## (b) Remove trailing state fragments from town strings and split obvious OCR
##     concatenations such as adjacent capital letters.
ua_dt[, OriginTown := trimws(gsub(",.*", "", `Origin Town`))]
ua_dt[, OriginTown := gsub("([A-Z])([A-Z])", "\\1 \\2", OriginTown)]
...
## (a) Compare degree-group shares by year and residency after excluding years
##     where degree-field parsing is known to be incomplete.
...
### i. read and examine recruiting inputs

## (a) Ozan's cleaned data provide a comparison benchmark for the 2017 scrape.
recruiting_dt <- fread(file = file.path(pathData, "recruiting_data_ozan.csv"))

## (b) The self-produced scrape is the source used by the newer town_matching
##     workflow; this legacy section keeps the original direct city-state checks.
panel_recruiting_dt <- fread(file = file.path(pathData, "full_ua_recruiting_scrape.csv"))

## (c) Drop rows without a parsed city because they cannot enter town-level merges.
panel_recruiting_dt <- panel_recruiting_dt[city != ""]

## (d) Apply manual city-name fixes identified during scrape validation.
panel_recruiting_dt[,city := fcase(city %flike% "A Shoreham", "Shoreham", 
                                   ...
                                   default = city)]
## (e) Standardize common abbreviations before comparing city strings.
...
## (f) Compare 2017 row counts against Ozan's cleaned benchmark. These calls are
##     intentionally left as console checks rather than stored outputs.
...
## (g) Plot visit volume over time for visual QA against expected scrape coverage.
...
panel_hs_dt <- fread(file = file.path(pathData, "bama_highschool_2011_2024.csv"))
...
### viii. exploratory recruiting regressions for appendix table

## (a) These legacy specifications use direct city-state merges. The production
##     town-level workflow now normalizes towns and writes audit files in
##     code/crossan/recruiting/town_matching before estimating the preferred
##     regressions.
...
#=====================================================================
# 4 - exploratory recruiting regressions with past enrollment controls
#=====================================================================

### i. past enrollment

## (a) Compare OOS town counts in 2017 and 2022, excluding Alabama and
##     international origins to focus on out-of-state recruiting markets.
...
### ii. build a wide town-year outcome panel for exploratory specifications

## (a) Compute town-level OOS counts and shares for each pre/post year.
...
## (b) Merge the yearly town tables into one wide panel.
...
## (c) Add high-school visit counts from the raw recruiting event file.
visit_counts_dt <- ua_hs_visit_dt[event_city != "", .N, by = .(event_city, event_state)]
setnames(visit_counts_dt, c("event_city", "event_state"), c("OriginTown", "OriginState"))
setnames(visit_counts_dt, "N", "VisitCount")
...
### i. read data 
linked_dt <- fread(file = file.path(pathData, "linked_commencement_revelio_profile_data.csv"))
...
## (a) linked data 
revelio_ua_link_dt <- fread(file = file.path(pathData, "linked_commencement_revelio_profile_data.csv"))

## (b) first spell
first_spells_join_dt <- readRDS(file.path(pathRevelio, "first_spell_join.rds"))
