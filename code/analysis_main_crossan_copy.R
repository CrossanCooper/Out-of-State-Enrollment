...
# Author: Ryan Haygood / edited by Crossan Cooper
# Date: 2/12/25
...
dest <- readRDS(file.path(getwd(), "revelio_data", "first_spell_join.rds"))

# Read origin data
origin <- read.csv(file.path(getwd(), 'data','linked_commencement_revelio_profile_data.csv'))
...
iv <- readRDS(file.path(getwd(), 'data','selectivity_iv.rds')) %>%
...
# write_dta(share_ins, file.path(getwd(), 'data','share_ins.dta'))
...
data.frame(t = rep(2006:2023, 2),
  ...
  theme(panel.grid.major.y = element_line(color = 'gray80', linetype = 'dashed'),
        ...
        plot.title = element_text(hjust = 0.5),
        plot.caption = element_text(hjust = 0))
ggsave(file.path(getwd(),'figures','descriptive-figs','crossan_counterfactual.png'), width = 8, height = 4.5)
...
ef <- readRDS(file.path(getwd(), 'data','ef_by_state_panel.rds')) %>%
